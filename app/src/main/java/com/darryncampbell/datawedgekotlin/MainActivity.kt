...
class MainActivity : AppCompatActivity(), Observer, View.OnTouchListener {
    ...
    companion object {
        const val TAG = "CustomLog:MainActivity"
    ...
    private fun createDataWedgeProfile() {
        Log.i(TAG, "createDataWedgeProfile")
    ...
    private fun writeFile() {
        ...
        var objectOut2: ObjectOutputStream? = null
        try {
            val fileOut2 = applicationContext.openFileOutput(SCAN_HISTORY_XML, Activity.MODE_PRIVATE)
            var unString: String
            for (item in scans){
                unString = item.data + ";" + item.symbology + ";" + item.dateTime
                fileOut2.write(unString.toByteArray())
            }
        } catch (e: IOException) {
            e.printStackTrace()
        } finally {
